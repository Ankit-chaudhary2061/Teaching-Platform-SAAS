...
class TeacherController{
    static async createTecaher(req:IExtendedRequest, res :Response){
        try {
        const instituteNumber = req.user?.currentInstituteNumber;
        if (!instituteNumber) {
        return res.status(400).json({ message: "Institute number is missing" });
        }
        const {teacherName, teacherEmail, teacherJoinedDate, teacherExpertise, teacherSalary, teacherPhoneNumber, courseId} =req.body
        if(!teacherName || ! teacherEmail || ! teacherJoinedDate || !teacherExpertise || !teacherSalary || ! teacherPhoneNumber){
        ...
        await sequelize.query(`INSERT INTO teacher_${instituteNumber}(teacherName, teacherEmail, teacherJoinedDate, teacherExpertise, teacherSalary, teacherPhoneNumber, teacherPhoto, teacherPassword) VALUES(?,?,?,?,?,?,?,?)`,{
            replacements:[teacherName, teacherEmail, teacherJoinedDate, teacherExpertise, teacherSalary, teacherPhoneNumber, teacherPhoto, password.hashedVersion]
        })
